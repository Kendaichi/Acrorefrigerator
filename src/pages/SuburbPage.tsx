...
const SuburbPage = () => {
  ...
  if (!result) return <Navigate to="/locations" replace />;
  ...
  return (
    <Layout>
      <SEO
        ...
        canonical={`/locations/${city.slug}/${suburb.slug}`}
      />
      ...
      <div className="bg-secondary px-6 py-3">
        <div className="container-narrow">
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem>
                <BreadcrumbLink asChild><Link to="/locations">Locations</Link></BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink asChild><Link to={`/locations/${city.slug}`}>{city.name}</Link></BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
          ...
          <motion.div className="max-w-3xl" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button asChild size="lg" variant="outline">
                <Link to="/contact-us">Get a Quote <ArrowRight className="w-4 h-4 ml-2" /></Link>
              </Button>
            </div>
          </motion.div>
        </div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {suburb.nearbySuburbs.map((name) => {
              ...
              return linked ? (
                <Link
                  key={name}
                  to={`/locations/${city.slug}/${linked.slug}`}
